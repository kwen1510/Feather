...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    ...
    const connectToAbly = async () => {
      try {
        ...
        // Create broadcast channel for teacher-to-all communication
        // MUST be created BEFORE connection handlers so it's available in reconnection logic
          ...
          if (hasInitiallyConnected) {
            ...
            try {
              ...
              setTimeout(async () => {
                ...
                const connectedMembers = await broadcastCh.presence.get();
                ...
                if (studentCount === 0) {
                  ...
                  // Publish to broadcast channel (all students receive it)
                  broadcastCh.publish('request-student-strokes', {
                    timestamp: Date.now(),
                  });
                }
              }, 500);
        ...
        // Note: Channels already created above (broadcastCh) before connection handlers

        // Helper function to subscribe to student channel (avoid duplicate subscriptions)
        const subscribeToStudentChannel = (studentCh, studentIdParam) => {
          studentCh.subscribe('student-layer', (message) => {
          ...
          studentCh.subscribe('student-visibility', (message) => {
          ...
          studentCh.subscribe('request-current-state', (message) => {
            ...
            setTimeout(() => {
              try {
                ...
                studentCh.publish('sync-full-state', {
          ...
          studentCh.subscribe('request-teacher-annotations', async (message) => {
            const requestingStudentId = message.data?.studentId;
            ...
            try {
              ...
              setTimeout(() => {
                if (studentCh && studentCh.state === 'attached') {
                  studentCh.publish('response-teacher-annotations', {
          ...
          studentCh.subscribe('response-student-strokes', (message) => {
        ...
        // Listen for presence events (student connect/disconnect) on broadcast channel
        broadcastCh.presence.subscribe('enter', (member) => {
          ...
          // Create individual channel for this student if it doesn't exist
          setStudentChannels(prevChannels => {
            if (!prevChannels[incomingStudentId]) {
              console.log(`📡 Creating individual channel for student ${incomingStudentId}`);
              const studentCh = ablyClient.channels.get(`${roomId}-${incomingStudentId}`);
              
              // Subscribe to all student messages using helper function
              subscribeToStudentChannel(studentCh, incomingStudentId);

              return {
                ...prevChannels,
                [incomingStudentId]: studentCh
              };
            }
            return prevChannels;
          });
          ...
          // Send current shared image if exists (via individual channel)
          setTimeout(() => {
            try {
              const studentCh = studentChannelsRef.current?.[incomingStudentId];
              if (!studentCh || studentCh.state !== 'attached') {
                console.warn('⚠️ Cannot send sync-full-state: channel not ready');
                return;
              }
          ...
          }, 500); // Longer delay to ensure channel subscriptions are set up
        });

        broadcastCh.presence.subscribe('leave', (member) => {
          if (member.clientId !== clientId && member.clientId.includes('student')) {
            ...
            // Cleanup individual channel for this student
            setStudentChannels(prevChannels => {
              if (prevChannels[leavingStudentId]) {
                console.log(`📡 Cleaning up individual channel for student ${leavingStudentId}`);
                const studentCh = prevChannels[leavingStudentId];
                studentCh.unsubscribe(); // Unsubscribe from all events
                const { [leavingStudentId]: removed, ...remaining } = prevChannels;
                return remaining;
              }
              return prevChannels;
            });
        ...
        // Note: Individual student message handlers (student-layer, student-visibility, etc.) 
        // are now subscribed dynamically in presence.enter when creating individual channels
        // This eliminates broadcast messages and routes everything through targeted channels

        // Enter presence with teacher role on broadcast channel
        await broadcastCh.presence.enter({
          role: 'teacher',
          timestamp: Date.now()
        });
        ...
        const existingMembers = await broadcastCh.presence.get();

        // Process existing members and create individual channels for them
        ...
        existingMembers.forEach(member => {
          ...
          console.log(`📡 Creating individual channel for existing student ${memberStudentId}`);
          const studentCh = ablyClient.channels.get(`${roomId}-${memberStudentId}`);
          
          // Subscribe to all student messages using helper function
          subscribeToStudentChannel(studentCh, memberStudentId);

          newChannels[memberStudentId] = studentCh;
        ...
        // Update state with all existing students
        setStudents(prev => ({
          ...prev,
          ...currentStudents
        }));

        // Update state with all individual channels
        setStudentChannels(newChannels);
        ...
        if (isPageRefresh) {
          setTimeout(async () => {
            try {
              ...
              setTimeout(async () => {
                if (broadcastCh && broadcastCh.state === 'attached') {
                  // Check number of connected students before requesting strokes
                  const connectedMembers = await broadcastCh.presence.get();
                  const connectedStudents = connectedMembers.filter(member =>
                    member.clientId &&
    ...
    return () => {
      ...
      // Cleanup all individual student channels
      Object.values(studentChannelsRef.current).forEach(studentCh => {
        try {
          studentCh.unsubscribe();
        } catch (error) {
          console.error('Error unsubscribing from student channel:', error);
        }
      });
  ...
  const handleAnnotate = async (annotations) => {
    ...
    // Use persistent studentId for storage and channel routing
    ...
    // Get individual channel for this student
    const studentCh = studentChannelsRef.current?.[persistentStudentId];
    if (!studentCh) {
      console.error('❌ No channel found for student', persistentStudentId);
      return;
    }

    // Publish annotations to individual channel (only this student receives)
    studentCh.publish('teacher-annotation', {
      annotations: annotations,
      teacherId: clientId,
      timestamp: Date.now(),
    });
